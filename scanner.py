"""
15-Minute Inside-Bar Setup Scanner — Nifty 50
------------------------------------------------
Pattern:
...
def fetch_nifty50_symbols():
    ...
    if len(symbols) != 50 or len(set(symbols)) != 50:
        raise ValueError(f"Expected 50 unique Nifty 50 symbols, received {len(symbols)}")
    return symbols


def save_nifty50_symbols(symbols):
    with open(UNIVERSE_FILE, "w") as f:
        json.dump(symbols, f)


def load_nifty50_symbols(force_refresh=False):
    if force_refresh:
        symbols = fetch_nifty50_symbols()
        save_nifty50_symbols(symbols)
        print(f"Saved latest Nifty 50 universe ({len(symbols)} symbols).")
        return symbols

    if os.path.exists(UNIVERSE_FILE):
        with open(UNIVERSE_FILE) as f:
            symbols = json.load(f)
        if len(symbols) == 50 and len(set(symbols)) == 50:
            return symbols
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--refresh-universe",
        action="store_true",
        help="Fetch and save the latest Nifty 50 constituents, then exit",
    )
    args = parser.parse_args()
    if args.refresh_universe:
        load_nifty50_symbols(force_refresh=True)
    else:
        run()
